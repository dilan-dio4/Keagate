...
class Dash extends GenericWallet_1.default {
    ...
    async getBalance() {
        const { data: { confirmed_balance, unconfirmed_balance } } = await (0, src_1.fGet)(`https://chain.so/api/v2/get_address_balance/DASH/${this.publicKey}`);
        return {
            result: {
    ...
    async sendTransaction(destination, amount) {
        ...
        const { data: { txs } } = await (0, src_1.fGet)(`https://chain.so/api/v2/get_tx_unspent/DASH/${this.publicKey}`);
        let totalBalance = 0;
        ...
        const { result } = await (0, src_1.fPost)(process.env.DASH_RPC_URL, {
            "jsonrpc": "2.0",
