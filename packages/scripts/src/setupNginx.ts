...
export default async function setupNginx(): Promise<Partial<MyConfig>> {
    if (!opts().dryrun) {
        try {
            await spawnAsync('docker', `run -d --network host --name keagate-nginx -v ${process.env.HOME || "~"}/Keagate/packages/scripts/assets/default.conf:/etc/nginx/conf.d/default.conf nginx:mainline`.split(" "));
        } catch (error) {
